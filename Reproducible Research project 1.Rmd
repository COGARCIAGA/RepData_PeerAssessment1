...
```{r, echo = TRUE}
activity_means_steps <- with(activity,
                                     aggregate(steps,
                                               by = list(date), FUN = mean, na.rm = TRUE))
names(activity_means_steps) <- c("date", "action")
png("steps_means_steps_plot2.png")
ggplot(activity_means_steps,aes(y = action , x = date), col= "red")+
        geom_bar(stat="identity")+
        theme_bw() + guides(fill="none")+
        ylab("Actions")+
        xlab("Weekday")+
        ggtitle("Total number of steps taken per weekday")
dev.off()
head(activity_means_steps)
```
...
```{r, echo = TRUE}
### The 5-minute interval that, on average, contains the maximum number of steps
Factor_interval <- as.factor(activity$interval)
activity_means_steps_per_interval <- with(activity,
                                      aggregate(steps,
                                                by = list(interval), FUN = mean, na.rm = TRUE))
names(activity_means_steps_per_interval) <- c("interval", "action")
activity_means_steps_per_interval <- activity_means_steps_per_interval[with(activity_means_steps_per_interval, order(-activity_means_steps_per_interval$action)),]
activity_means_steps_per_interval <- cbind.data.frame(activity_means_steps_per_interval, Factor_interval)
png("steps_means_steps_per_interval_plot3.png")
ggplot(activity_means_steps_per_interval,aes(y = action , x = Factor_interval))+
        geom_bar(stat="identity")+
        theme_bw() + guides(fill="none")+
        ylab("Actions")+
        xlab("Interval")+
        ggtitle("Total number of steps taken per interval")
dev.off()
```
...
activity_total_steps_witout_NA <- with(activity_clean,
                             aggregate(steps,
                                       by = list(date), FUN = sum, na.rm = TRUE))
names(activity_total_steps_witout_NA) <- c("moment", "action")
head(activity_clean)
```
...
png("steps_total_steps_witout_NA_plot4.png")
ggplot(activity_total_steps_witout_NA,aes(y = action , x = moment), col= "red")+
        geom_bar(stat="identity")+
        theme_bw() + guides(fill="none")+
        ylab("Actions")+
        xlab("Date")+
...
png("activity_different_steps_type_day_Plot5.png")
